...
} from "./PositionAnimation";
import useOrangeCircleAnimation from "./OrangeCircleAnimation";
import {usePage} from "@/provider/PageProvider";
import {PAGE_STATE} from "@/Schema";
import { useWindowDimensions } from "@/provider/WindowSizeProvider";
...
// children を含めた型にラップしたバージョン
const AnimatedDiv = animated.div as React.FC<
  React.PropsWithChildren<React.ComponentProps<typeof animated.div>>
>;
...
const ConnectionCircles: React.FC = () => {
  const { width, height } = useWindowDimensions(); // width/height: number を想定
  ...
  const useCirclesAnimation = (d: CircleDef) => {
    return {
      positionAnimation: usePositionAnimation(
        d,
        cx,
        cy,
        sizeRatio,
        pageNum as PAGE_STATE
      ),
  ...
  const centerCircle = useOrangeCircleAnimation(
    cx,
    cy,
    sizeRatio,
    pageNum as PAGE_STATE,
  ...
  return (
    <div>
      {animations.map(({ positionAnimation, boundingAnimation, color }, index) => (
        <AnimatedDiv
          key={index}
          style={{
            position: "absolute",
